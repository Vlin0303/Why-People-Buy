...
export default function CampaignCard({ campaign, index = 0 }) {
  ...
  const psychItems = getPsychologyItems(campaign.relatedPsychologyIds || []).slice(0, 3);
        ...
        >
          <div>
            <div className="flex items-center gap-3 mb-4">
              <p className="label">{String(campaign.date || "")}</p>
              {(campaign.market || []).slice(0, 1).map((m) => (
            ...
            <h3 className="text-xl font-light tracking-tight mb-3">
              {getLocalizedText(campaign.title, lang)}
            </h3>
